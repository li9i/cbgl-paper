...
\begin{customcnj}{C}
  ...
  Let $\mathcal{V} = \{\hat{\bm{p}}_i \in \mathcal{H}: \|\bm{p}-\hat{\bm{p}}_i\|_2 < \delta_0 \text{ and } \psi(\mathcal{S}_R,\mathcal{S}_V^{\bm{M}}(\hat{\bm{p}}_i)) < \psi_0\}$. Without loss of generality there exist $\delta_0$,
  $\psi_0$ $\in \mathbb{R}_{> 0}$ such that for all $\hat{\bm{p}}_\mathcal{V} \in \mathcal{V}$
  \begin{align}
