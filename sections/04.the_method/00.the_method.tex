...
estimate be in a neighbourhood of a 2D LIDAR sensor's pose within a given map
(Def. \ref{def:definition_0}); then the value of the CAER metric (Def.
...
map (Def. \ref{def:definition_2}) is simultaneously proportional to both the
estimate's location error and orientation error. In other words comparing any
two pose estimates residing in a neighbourhood of the sensor's true pose in
terms of a scalar computable value that is associated with them is enough to
establish a pose error hierarchy between them. This relationship of
proportionality has been discovered and successfully exploited in the context
of non-global localisation, i.e. production of lidar odometry
\cite{Filotheou2022f} and pose tracking \cite{Filotheou2023a}.
...
\begin{figure}\vspace{-1.5cm}
  ...
  \caption{\small Top: without loss o generality, a typical plot of the
           ...
           $10^6$ hypotheses on environment WAREHOUSE (Fig. \ref{fig:face} and
           section \ref{subsec:exp_b}). Bottom: focused view on
           hypotheses with location error close to the origin}
  \vspace{-0.5cm}
  \label{fig:motivation_caer}
\end{figure}

%THE HYPOTHESIS IS ... (we say that in a neighbourhood all is well. the hyp. is the generalisation)

%an arbitrary number of
%pose estimates. % ara...with enough estimates we can find out the sensor's pose
