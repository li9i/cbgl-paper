...
\begin{customcnj}{C}
  ...
  %and
  %$f_{\texttt{r}}^{\bm{M}}$ be the corresponding $\texttt{r}$-field (def. \ref{def:definition_5})
...
$\mathcal{X}$, and $\mathcal{W}$ such that the error of pose estimates in
set $\mathcal{V}$ and their corresponding CAER values %$f_{\texttt{r}}^{\bm{M}}(\mathcal{V})$
are simultaneously lower than those of estimates in set $\mathcal{X}$.
%$f_{\psi}^{\bm{M}}(\mathcal{V} \cup \mathcal{W}) < f_{\psi}^{\bm{M}}(\mathcal{X})$ and
%$f_{\texttt{r}}^{\bm{M}}(\mathcal{V}) < f_{\texttt{r}}^{\bm{M}}(\mathcal{X} \cup \mathcal{W})$.
%$f_{\psi}^{\bm{M}}(\mathcal{V}) < f_{\psi}^{\bm{M}}(\mathcal{X})$ and
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \FOR {$k \leftarrow 0,1,\dots,|\mathcal{H}_1|-1$}
      \STATE $\hat{\bm{h}}^\prime \leftarrow \texttt{sm2}(\mathcal{S}_R, \bm{M}, \mathcal{H}_1[k])$ \hfill {\small (Alg. \ref{alg:algorithm_sm2} or e.g. \texttt{x1} \cite{Filotheou2023a}})
